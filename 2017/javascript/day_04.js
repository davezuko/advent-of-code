...
const fs = require('fs')
const INPUT = fs.readFileSync('../inputs/day_04.txt', 'utf8')
  .split('\n')
  .filter(Boolean)
  .join('\n')
...
const isValidPassphrase_ = phrase => {
  ...
  return new Set(_.map(w => [...w].sort().join(''), words)).size === words.length
}

const exercise_02 = _.pipe([_.split('\n'), _.filter(isValidPassphrase_), _.length])
